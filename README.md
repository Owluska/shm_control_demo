...
The first part of the plot shows normal closed-loop behavior: the controller tries to make the simulated vehicle speed follow the target speed generated by the planner.

The gap between target speed and vehicle speed is expected because the controller is intentionally simple: it is only a proportional controller, and the mock vehicle model has inertia and drag. The throttle command is also limited, so the vehicle cannot instantly follow fast target changes.

The second part of the plot shows safety behavior. After the planner or other processes are stopped, the controller detects stale input data and switches to `safeStop()`. This sets throttle to zero and brake to one, so the simulated vehicle speed falls to zero even though the last target value remains visible in shared memory.