...
inline SlotSnapshot<T> readSlot(const SeqlockSlot<T>& slot) {
  ...
  while (true) {
    ...
    if (seq_before == seq_after) {
      return SlotSnapshot<T>{.data = copy, .seq = seq_after};
    }
  }
}

inline bool checkSharedMemoryLayout(const SharedMemoryLayout* shm) {
  return shm != nullptr && shm->magic == SHM_MAGIC &&
...
struct SharedMemoryMapping {
  ...
  SharedMemoryLayout* layout() { return static_cast<SharedMemoryLayout*>(ptr); }
  ...
  ~SharedMemoryMapping() { closeMapping(); }
...
inline SharedMemoryMapping openExistingSharedMemory() {
  ...
  mapping.ptr = mmap(nullptr, mapping.size, PROT_READ | PROT_WRITE, MAP_SHARED,
                     mapping.fd, 0);
