...
int main() {
  try {
    ...
    while (true) {
      ...
      std::cout << "target_seq=" << target.seq
      ...
      log_file << t_ns << ',' << target.seq << ','
               << ageMs(target.data.timestamp_ns) << ','
               << target.data.target_speed_mps << ',' << state.seq << ','
               << ageMs(state.data.timestamp_ns) << ',' << state.data.speed_mps
               << ',' << command.seq << ',' << ageMs(command.data.timestamp_ns)
               << ',' << command.data.throttle << ',' << command.data.brake
